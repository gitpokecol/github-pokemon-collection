...
async def test_get_pokedex__valid_request__responses_pokedex(
    ...
    # given
    pokedex_items = [
        create_pokedex_item(type=PokemonType.Abomasnow, owner=user),
        create_pokedex_item(type=PokemonType.Cacnea, owner=user),
    ]
    session.add_all(pokedex_items)
    ...
    assert response.json() == {
        "items": [{"pokemon_type": int(PokemonType.Abomasnow)}, {"pokemon_type": int(PokemonType.Cacnea)}]
    }
